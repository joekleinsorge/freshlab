...
Last updated: 2026-08-31 15:46 EDT
...
|---|---|---:|---|---|---|---|
| HW-001 | metal1 NVMe | Critical | Failed; disabled | The NVMe controller dropped from PCIe under write load. Kernel reported that the controller was down and failed D3cold-to-D0 power transitions. Its filesystem became emergency read-only. | The NVMe datastore was removed from `/etc/fstab` and Freshlab automation. K3s and etcd use the surviving root disk. | Replace/reseat the NVMe device or repair its PCIe/power-state problem, then burn-in test it before reuse. Do not place cluster data on it meanwhile. |
| HW-002 | metal1 root disk / etcd | High | Active risk | etcd reads repeatedly took 0.1-10+ seconds and metal1 became `NotReady` during reconciliation. The root disk showed sustained I/O wait and cannot reliably handle this control-plane workload. | K3s was recovered and metal1 is `Ready` but cordoned (`SchedulingDisabled`). The metal2/metal3 control plane retains quorum. | Move metal1's etcd data to proven SSD/NVMe storage, or replace metal1 as an etcd member with a suitably fast node. |

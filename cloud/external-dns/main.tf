...
resource "cloudflare_argo_tunnel" "freshlab_tunnel" {
  account_id = var.cloudflare_account_id
  name       = "${var.environment}-freshlab-tunnel"
...
resource "cloudflare_record" "tunnel" {
  ...
  type    = "CNAME"
  name    = "${var.environment}-freshlab-tunnel"
  value   = "${cloudflare_argo_tunnel.freshlab_tunnel.id}.cfargotunnel.com"
  proxied = false
  ttl     = 1 # Auto
}
...
resource "kubernetes_secret" "cloudflared_credentials" {
  ...
  data = {
    "credentials.json" = jsonencode({
      ...
      TunnelName   = cloudflare_argo_tunnel.freshlab_tunnel.name
      TunnelID     = cloudflare_argo_tunnel.freshlab_tunnel.id
